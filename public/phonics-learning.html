...
<title>拼读乐园 · PhonicsFun — 小学生自然拼读学习</title>
...
<style>
:root{
  --sky:#7ec8ff; --sky-deep:#4ea8e8;
  --sun:#ffd84d; --sun-deep:#ffb800;
  --grass:#6dd47e; --grass-deep:#3cb878;
  --coral:#ff8a6b; --coral-deep:#ff6b4a;
  --grape:#b794f6; --grape-deep:#9f7aea;
  --cream:#fff8e7; --paper:#ffffff;
  --ink:#2d3142; --ink-soft:#5a6072;
  --shadow:0 6px 0 rgba(0,0,0,.08), 0 12px 24px rgba(0,0,0,.10);
  --shadow-sm:0 4px 0 rgba(0,0,0,.06), 0 8px 16px rgba(0,0,0,.08);
  --radius:20px;
}
*{margin:0;padding:0;box-sizing:border-box;-webkit-tap-highlight-color:transparent}
html{scroll-behavior:smooth}
/* 移动端触摸优化：消除 300ms 点击延迟 */
button,[onclick],.sound-card,.word-chip,.level-card,.quiz-opt,.sentence-card,.q-sound,.wf-play,.next-btn,.close-btn,.btn-start{
...
h1,h2,h3,.display{font-family:'Baloo 2','PingFang SC',sans-serif}
...
.nav{
  ...
  border-bottom:3px solid rgba(126,200,255,.25);
  padding:10px 24px;
  display:flex;align-items:center;justify-content:space-between;
}
.brand{display:flex;align-items:center;gap:10px;font-family:'Baloo 2',sans-serif;font-weight:800;font-size:22px}
.brand .logo{font-size:28px}
.brand .en{color:var(--sky-deep)}
.nav-stats{display:flex;gap:16px;align-items:center;font-size:14px;font-weight:700}
.star-count{background:var(--sun);color:var(--ink);padding:4px 14px;border-radius:30px;box-shadow:var(--shadow-sm)}
.progress-mini{width:120px;height:10px;background:#e8e0cc;border-radius:10px;overflow:hidden}
.progress-mini span{display:block;height:100%;background:linear-gradient(90deg,var(--grass),var(--grass-deep));border-radius:10px;transition:width .5s}

/* ===== 英雄区 ===== */
.hero{
  position:relative;
  background:linear-gradient(180deg,#a8dcff 0%, #c6e8ff 40%, var(--cream) 100%);
...
.hero::before{
  ...
  background:radial-gradient(circle,rgba(255,216,77,.5),transparent 70%);border-radius:50%;
}
.cloud{position:absolute;color:#fff;opacity:.85;font-size:40px;animation:float 8s ease-in-out infinite}
.cloud.c1{top:30px;left:8%;animation-delay:0s}
.cloud.c2{top:80px;right:10%;font-size:32px;animation-delay:2s}
.cloud.c3{top:140px;left:20%;font-size:28px;animation-delay:4s}
...
.hero h1 .cn{display:block;color:var(--ink)}
.hero h1 .en{display:block;font-size:.55em;color:var(--sky-deep);letter-spacing:2px}
.hero .tag{
...
.hero .tags{display:flex;gap:10px;justify-content:center;flex-wrap:wrap;margin-top:18px;position:relative;z-index:2}
.hero .tags span{
  background:#fff;padding:6px 16px;border-radius:30px;font-weight:700;font-size:14px;
...
.btn-start{
  ...
  padding:18px 44px;border-radius:40px;border:none;cursor:pointer;
  box-shadow:0 6px 0 #d94a2f,0 12px 24px rgba(255,107,74,.4);
  margin-top:28px;transition:transform .15s,box-shadow .15s;position:relative;z-index:2;
  min-height:56px;
}
.btn-start:hover{transform:translateY(-2px);box-shadow:0 8px 0 #d94a2f,0 16px 28px rgba(255,107,74,.45)}
.btn-start:active{transform:translateY(3px);box-shadow:0 3px 0 #d94a2f}

.mascot{font-size:72px;margin-top:8px;animation:bounce 2s ease-in-out infinite;position:relative;z-index:2}
@keyframes bounce{0%,100%{transform:translateY(0)}50%{transform:translateY(-12px)}}

/* ===== 学习路径 ===== */
.path-section{max-width:1080px;margin:0 auto;padding:48px 20px}
.section-title{text-align:center;margin-bottom:32px}
.section-title h2{font-size:clamp(28px,5vw,42px);font-weight:800}
.section-title p{color:var(--ink-soft);font-size:16px;margin-top:6px}

.path-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:20px}
.level-card{
  background:var(--paper);border-radius:var(--radius);padding:24px;cursor:pointer;
  box-shadow:var(--shadow);transition:transform .2s,box-shadow .2s;
  border:3px solid transparent;position:relative;overflow:hidden;
}
.level-card:hover{transform:translateY(-4px);box-shadow:0 10px 0 rgba(0,0,0,.08),0 18px 36px rgba(0,0,0,.12)}
.level-card.active{border-color:var(--sky-deep)}
.level-card .num{
  position:absolute;top:-8px;right:16px;font-family:'Baloo 2',sans-serif;
  font-size:64px;font-weight:800;opacity:.08;line-height:1;
}
.level-card .icon{font-size:40px;margin-bottom:8px}
.level-card h3{font-size:20px;font-weight:700;margin-bottom:4px}
.level-card .en-name{font-size:13px;color:var(--sky-deep);font-weight:700;text-transform:uppercase;letter-spacing:1px}
.level-card .desc{font-size:14px;color:var(--ink-soft);margin-top:8px}
.level-card .badge{position:absolute;top:12px;right:12px;font-size:20px}
.level-card.locked{opacity:.55;cursor:not-allowed}
.level-card.locked:hover{transform:none}
.level-card.done{background:linear-gradient(135deg,#f0fff4,#e6ffe6)}

/* 主题色 */
.lc-1{--c:var(--sky-deep)} .lc-2{--c:var(--sun-deep)}
.lc-3{--c:var(--grass-deep)} .lc-4{--c:var(--coral-deep)}
.lc-5{--c:var(--grape-deep)} .lc-6{--c:var(--sky-deep)}
.lc-7{--c:var(--coral-deep)}
.level-card .icon{color:var(--c)}
.level-card .en-name{color:var(--c)}
...
.learn-panel.show{display:block;animation:slideUp .4s ease}
...
.panel-header .ph-icon{font-size:44px;float:left;margin-right:16px}
.panel-header h2{font-size:28px;font-weight:800}
.panel-header .ph-en{font-size:14px;opacity:.9;text-transform:uppercase;letter-spacing:1px}
.panel-header .ph-tip{font-size:14px;opacity:.9;margin-top:6px}
.panel-header .close-btn{
  position:absolute;top:20px;right:24px;background:rgba(255,255,255,.25);
  border:none;color:#fff;width:38px;height:38px;border-radius:50%;cursor:pointer;
  font-size:20px;font-weight:700;transition:background .2s;
}
.panel-header .close-btn:hover{background:rgba(255,255,255,.4)}

.panel-body{padding:28px 32px 36px}

/* 声音卡片网格 */
.sound-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:16px}
.sound-card{
  background:var(--cream);border-radius:16px;padding:20px 12px;text-align:center;
  cursor:pointer;transition:transform .15s,box-shadow .15s;
  box-shadow:var(--shadow-sm);border:3px solid transparent;user-select:none;
}
.sound-card:hover{transform:translateY(-3px);box-shadow:0 7px 0 rgba(0,0,0,.06),0 14px 20px rgba(0,0,0,.1)}
.sound-card.playing{border-color:var(--grass-deep);animation:pulse .5s}
@keyframes pulse{0%{transform:scale(1)}50%{transform:scale(1.06)}100%{transform:scale(1)}}
.sound-card .letter{font-family:'Baloo 2',sans-serif;font-size:40px;font-weight:800;color:var(--ink);line-height:1}
.sound-card .ipa{font-size:18px;color:var(--coral-deep);font-weight:700;margin-top:4px}
.sound-card .word{font-size:15px;font-weight:700;color:var(--sky-deep);margin-top:8px}
.sound-card .emoji{font-size:28px;margin-top:6px;display:block}
.sound-card .speak-icon{font-size:14px;color:var(--grass-deep);margin-top:6px}

/* 词族组 */
.word-family{
  background:var(--cream);border-radius:16px;padding:20px;margin-bottom:18px;
  box-shadow:var(--shadow-sm);
}
.word-family .wf-head{display:flex;align-items:center;gap:10px;margin-bottom:14px}
.word-family .wf-pattern{
  font-family:'Baloo 2',sans-serif;font-weight:800;font-size:24px;
  background:var(--sun);padding:4px 16px;border-radius:30px;box-shadow:var(--shadow-sm);
}
.word-family .wf-ipa{font-size:16px;color:var(--coral-deep);font-weight:700}
.word-family .wf-play{
  margin-left:auto;background:var(--grass);border:none;color:#fff;
  padding:6px 16px;border-radius:30px;font-weight:700;cursor:pointer;font-size:13px;
  box-shadow:0 3px 0 var(--grass-deep);transition:transform .1s;
}
.word-family .wf-play:active{transform:translateY(2px);box-shadow:0 1px 0 var(--grass-deep)}
.word-list{display:flex;flex-wrap:wrap;gap:10px}
.word-chip{
  background:#fff;padding:8px 18px;border-radius:30px;font-weight:700;font-size:17px;
  cursor:pointer;box-shadow:0 3px 0 rgba(0,0,0,.06);transition:transform .1s,background .2s;
  display:flex;align-items:center;gap:6px;
}
.word-chip:hover{background:var(--sun)}
.word-chip:active{transform:translateY(2px)}
.word-chip .chip-emoji{font-size:18px}

/* 句子卡 */
.sentence-card{
  background:var(--cream);border-radius:16px;padding:24px;margin-bottom:16px;
  box-shadow:var(--shadow-sm);cursor:pointer;transition:transform .15s;
  border-left:6px solid var(--grass-deep);
}
.sentence-card:hover{transform:translateX(4px)}
.sentence-card .sent-en{font-size:22px;font-weight:700;line-height:1.5}
.sentence-card .sent-en .hi{background:var(--sun);padding:2px 8px;border-radius:6px}
.sentence-card .sent-cn{font-size:15px;color:var(--ink-soft);margin-top:8px}
.sentence-card .play-hint{font-size:13px;color:var(--grass-deep);font-weight:700;margin-top:10px}

/* 练习题 */
.quiz-section{margin-top:32px}
.quiz-title{font-size:22px;font-weight:800;margin-bottom:16px;display:flex;align-items:center;gap:8px}
.quiz-card{
  background:linear-gradient(135deg,#fffbe6,#fff8e7);border-radius:16px;padding:24px;
  box-shadow:var(--shadow-sm);margin-bottom:16px;
}
.quiz-q{font-size:18px;font-weight:700;margin-bottom:16px}
.quiz-q .q-sound{
  display:inline-flex;align-items:center;gap:6px;background:var(--sky);color:#fff;
  padding:4px 14px;border-radius:30px;cursor:pointer;font-size:16px;box-shadow:0 3px 0 var(--sky-deep);
}
.quiz-options{display:grid;grid-template-columns:repeat(auto-fill,minmax(120px,1fr));gap:12px}
.quiz-opt{
  background:#fff;padding:14px;border-radius:14px;text-align:center;cursor:pointer;
  font-weight:700;font-size:18px;box-shadow:0 4px 0 rgba(0,0,0,.06);transition:transform .1s;
  border:3px solid transparent;
}
.quiz-opt:hover{transform:translateY(-2px)}
.quiz-opt:active{transform:translateY(2px)}
.quiz-opt.correct{background:#d4f5dd;border-color:var(--grass-deep);animation:shake .4s}
.quiz-opt.wrong{background:#ffd4d4;border-color:var(--coral-deep);animation:shake .4s}
@keyframes shake{0%,100%{transform:translateX(0)}25%{transform:translateX(-6px)}75%{transform:translateX(6px)}}
.quiz-feedback{margin-top:14px;font-weight:700;font-size:16px;min-height:24px}
.quiz-feedback.right{color:var(--grass-deep)}
.quiz-feedback.wrong{color:var(--coral-deep)}

/* 完成奖励 */
.complete-banner{
  text-align:center;padding:32px;background:linear-gradient(135deg,var(--sun),var(--sun-deep));
  border-radius:16px;margin-top:24px;box-shadow:var(--shadow);
}
.complete-banner .stars{font-size:40px;letter-spacing:4px}
.complete-banner h3{font-size:24px;margin:8px 0}
.complete-banner p{font-size:15px}
.next-btn{
  background:#fff;color:var(--ink);border:none;padding:12px 30px;border-radius:30px;
  font-weight:700;font-size:16px;cursor:pointer;margin-top:14px;box-shadow:var(--shadow-sm);
...
/* ===== 播放指示器 ===== */
.play-indicator{
  position:fixed;bottom:24px;right:24px;background:#fff;border-radius:50%;
  width:56px;height:56px;display:none;align-items:center;justify-content:center;
  box-shadow:var(--shadow);z-index:200;font-size:24px;
}
.play-indicator.show{display:flex;animation:pop .3s}
@keyframes pop{from{transform:scale(0)}to{transform:scale(1)}}
.play-indicator .wave{position:absolute;width:100%;height:100%;border-radius:50%;border:3px solid var(--grass);animation:wave 1s ease-out infinite}
@keyframes wave{from{transform:scale(1);opacity:1}to{transform:scale(1.8);opacity:0}}

/* ===== 页脚 ===== */
.footer{
  background:var(--ink);color:#a0a8bd;text-align:center;padding:32px 20px;font-size:14px;
}
.footer .ref{max-width:760px;margin:0 auto 12px;font-size:12px;line-height:1.8;text-align:left}
.footer .ref a{color:var(--sky);text-decoration:none}

/* ===== 响应式 ===== */
@media(max-width:640px){
  .nav{padding:8px 14px}
  .brand{font-size:18px}
  .nav-stats{gap:8px;font-size:12px}
  .progress-mini{width:70px}
  .hero{padding:32px 16px 48px}
  .panel-header{padding:20px}
  .panel-body{padding:20px}
  .sound-grid{grid-template-columns:repeat(auto-fill,minmax(110px,1fr));gap:10px}
  .sound-card{padding:14px 8px}
  .sound-card .letter{font-size:32px}
...
<style>.trae-browser-inspect-overlay{position:fixed;top:0;left:0;width:100%;height:100%;z-index:999997;pointer-events:auto;background:rgba(0,0,0,0)}.trae-browser-inspect-highlight-box{position:absolute;top:0;left:0;z-index:999998;display:none}.trae-browser-inspect-highlight-tag{position:absolute;left:0px;top:0px;height:18px;padding:0 6px;box-sizing:border-box;border-radius:4px;background-color:var(--vscode-icube-colorSuccessHover, "#2FB287");color:var(--vscode-icube-colorSnow, "#fff");font-size:12px;font-weight:400;display:none;align-items:center;z-index:999999}.trae-browser-inspect-highlight-box-show{display:flex}.trae-browser-inspect-select-button.trae-browser-inspect-highlight-box-show{display:flex !important}.trae-browser-inspect-select-button{position:absolute;top:0;left:0;background-color:var(--vscode-icube--bg-bg-base-secondary, "#222427");color:var(--vscode-icube--text-text-default-active, "#F5F9FE");font-size:11px;font-weight:500;border:1px solid var(--vscode-icube--border-neutral-l1, "#E0E2F217");border-radius:6px;padding:0 8px;height:32px;display:none;align-items:center;cursor:pointer;z-index:999999;gap:4px}.trae-browser-inspect-select-button svg,.trae-browser-inspect-select-button span{z-index:1}.trae-browser-inspect-select-highlight-box{background:rgba(0,0,0,0) !important}.trae-browser-inspect-select-highlight-box .trae-browser-inspect-highlight-tag{background-color:var(--vscode-icube--status-success-active, "#1E8B56")}.trae-browser-inspect-comment-card-container{position:fixed;top:0;left:0;z-index:9999999;pointer-events:auto}.trae-browser-inspect-comment-card-container .trae-browser-inspect-comment-card{display:flex;padding-top:0px !important;align-items:center;z-index:9999999 !important;background-color:var(--vscode-icube--bg-bg-menu)}*[contenteditable]{outline:none}.trae-browser-inspect-click-effect{position:fixed;width:40px;height:40px;margin-left:-20px;margin-top:-20px;border-radius:50%;background-color:var(--vscode-icube-colorSuccessHover, #2FB287);pointer-events:none;animation:trae-browser-inspect-ripple 500ms ease-out forwards;z-index:999999}@keyframes trae-browser-inspect-ripple{0%{transform:scale(0.3);opacity:.8}100%{transform:scale(2);opacity:0}}.trae-browser-inspect-ref-box{position:absolute;border:2px solid var(--vscode-icube-colorSuccessHover, "#2FB287");pointer-events:none;z-index:999998;animation:trae-browser-inspect-fadeInOut 2s ease-in-out forwards}.trae-browser-inspect-ref-label{position:absolute;top:-14px;left:0;height:14px;padding:0 4px;box-sizing:border-box;border-radius:2px;background-color:var(--vscode-icube-colorSuccessHover, "#2FB287");color:var(--vscode-icube-colorSnow, "#fff");font-size:9px;font-weight:500;display:flex;align-items:center;white-space:nowrap;z-index:999999}@keyframes trae-browser-inspect-fadeInOut{0%{opacity:0}10%{opacity:1}90%{opacity:1}100%{opacity:0}}.trae-browser-inspect-input-highlight{position:absolute;border:2px solid var(--vscode-icube-colorSuccessHover, "#2FB287");border-radius:4px;pointer-events:none;z-index:999998;box-shadow:0 0 8px var(--vscode-icube-colorSuccessHover, "#2FB287");animation:trae-browser-inspect-input-pulse 600ms ease-out forwards}@keyframes trae-browser-inspect-input-pulse{0%{opacity:0;box-shadow:0 0 0 var(--vscode-icube-colorSuccessHover, "#2FB287")}30%{opacity:1;box-shadow:0 0 12px var(--vscode-icube-colorSuccessHover, "#2FB287")}100%{opacity:0;box-shadow:0 0 0 var(--vscode-icube-colorSuccessHover, "#2FB287")}}.trae-browser-inspect-virtual-cursor{position:fixed;width:20px;height:20px;pointer-events:none;z-index:9999999;opacity:0;transition:opacity .2s ease}.trae-browser-inspect-virtual-cursor.trae-browser-inspect-cursor-visible{opacity:1}.trae-browser-inspect-virtual-cursor.trae-browser-inspect-cursor-idle{animation:trae-browser-inspect-cursor-breathe 2s ease-in-out infinite}.trae-browser-inspect-virtual-cursor svg{width:20px;height:20px;filter:drop-shadow(0 1px 2px rgba(0, 0, 0, 0.3))}@keyframes trae-browser-inspect-cursor-breathe{0%,100%{opacity:.7}50%{opacity:1}}.trae-browser-inspect-scroll-indicator{position:fixed;display:flex;align-items:center;justify-content:center;gap:6px;padding:8px 16px;border-radius:8px;background-color:rgba(0,0,0,.7);color:#fff;font-size:13px;font-weight:500;pointer-events:none;z-index:999999;animation:trae-browser-inspect-indicator-fade 800ms ease-out forwards;backdrop-filter:blur(4px)}.trae-browser-inspect-scroll-indicator svg{width:16px;height:16px;fill:#fff}.trae-browser-inspect-scroll-indicator.trae-scroll-up{top:12px;left:50%;transform:translateX(-50%)}.trae-browser-inspect-scroll-indicator.trae-scroll-down{bottom:12px;left:50%;transform:translateX(-50%)}.trae-browser-inspect-scroll-indicator.trae-scroll-left{left:12px;top:50%;transform:translateY(-50%)}.trae-browser-inspect-scroll-indicator.trae-scroll-right{right:12px;top:50%;transform:translateY(-50%)}@keyframes trae-browser-inspect-indicator-fade{0%{opacity:0;transform:translateX(-50%) scale(0.9)}15%{opacity:1;transform:translateX(-50%) scale(1)}85%{opacity:1}100%{opacity:0}}.trae-browser-inspect-scroll-indicator.trae-scroll-left,.trae-browser-inspect-scroll-indicator.trae-scroll-right{animation-name:trae-browser-inspect-indicator-fade-y}@keyframes trae-browser-inspect-indicator-fade-y{0%{opacity:0;transform:translateY(-50%) scale(0.9)}15%{opacity:1;transform:translateY(-50%) scale(1)}85%{opacity:1}100%{opacity:0}}.trae-browser-inspect-key-badge{position:fixed;bottom:60px;left:50%;transform:translateX(-50%);padding:8px 20px;border-radius:8px;background-color:rgba(0,0,0,.8);color:#fff;font-size:18px;font-weight:600;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,monospace;pointer-events:none;z-index:999999;animation:trae-browser-inspect-key-pop 500ms ease-out forwards;backdrop-filter:blur(4px);border:1px solid hsla(0,0%,100%,.15);box-shadow:0 4px 12px rgba(0,0,0,.3);letter-spacing:1px}@keyframes trae-browser-inspect-key-pop{0%{opacity:0;transform:translateX(-50%) translateY(10px) scale(0.9)}20%{opacity:1;transform:translateX(-50%) translateY(0) scale(1)}80%{opacity:1}100%{opacity:0;transform:translateX(-50%) translateY(-10px)}}.trae-browser-inspect-step-badge{position:fixed;top:12px;left:12px;padding:6px 12px;border-radius:6px;background-color:var(--vscode-icube-colorSuccessHover, #2FB287);color:#fff;font-size:12px;font-weight:600;pointer-events:none;z-index:999999;animation:trae-browser-inspect-step-slide 600ms ease-out forwards;box-shadow:0 2px 8px rgba(47,178,135,.3);max-width:280px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.trae-browser-inspect-step-badge.trae-step-old{opacity:.4;transform:translateY(0)}@keyframes trae-browser-inspect-step-slide{0%{opacity:0;transform:translateX(-20px)}20%{opacity:1;transform:translateX(0)}100%{opacity:1;transform:translateX(0)}}.trae-browser-inspect-drag-trail{position:fixed;top:0;left:0;width:100%;height:100%;pointer-events:none;z-index:999998}.trae-browser-inspect-drag-trail svg{width:100%;height:100%}.trae-browser-inspect-drag-trail .trae-drag-path{fill:none;stroke:var(--vscode-icube-colorSuccessHover, #2FB287);stroke-width:2;stroke-dasharray:8 4;opacity:.6;animation:trae-browser-inspect-dash 600ms linear forwards}.trae-browser-inspect-drag-trail .trae-drag-dot{fill:var(--vscode-icube-colorSuccessHover, #2FB287);filter:drop-shadow(0 0 4px rgba(47, 178, 135, 0.6))}.trae-browser-inspect-drag-trail .trae-drag-endpoint{fill:none;stroke:var(--vscode-icube-colorSuccessHover, #2FB287);stroke-width:2;opacity:.5}@keyframes trae-browser-inspect-dash{0%{stroke-dashoffset:200}100%{stroke-dashoffset:0}}.trae-browser-inspect-drag-trail.trae-drag-fade{animation:trae-browser-inspect-trail-fade 400ms ease-out forwards}@keyframes trae-browser-inspect-trail-fade{0%{opacity:1}100%{opacity:0}}</style><script type="module">(async () => {
  try {
    ...
    hmr.on('vite:afterUpdate', window['__ICUBE_WEBVIEW_HMR_CALLBACK_1785386415750']);
  } catch {}
...
.previewCommentCardSuccessText-s40BWG {
  ...
  try {
    const client = await import('/@vite/client');
    const hmr = client.createHotContext('hmr-handler/fake.js');
    hmr.on('vite:afterUpdate', window['__ICUBE_WEBVIEW_HMR_CALLBACK_1785386415755']);
  } catch {}
})();</script><style>.buttonsCard-bTXa5j {
  padding: var(--spacer-4);
  background: none;
...
[data-theme='dark-blue'], .vs-dark.solo-lite {
  ...
  --trae-layer-browser-inspect-overlay: 90000;
  --trae-layer-browser-inspect-highlight: 90010;
  --trae-layer-browser-inspect-action: 90020;
  --trae-layer-text-selection-action: 90030;
  --trae-layer-edit-tooltip: 100095;
}
:root,
[data-trae-theme='dark'] {
  ...
  --trae-region-outside-mask-bg: rgba(0, 0, 0, 0.32);
...
[data-trae-theme='light'] {
  ...
  --trae-annotation-marker-text: #ffffff;
  --trae-annotation-region-border: var(--trae-status-primary-default);
  --trae-region-outside-mask-bg: rgba(0, 0, 0, 0.18);
  --trae-annotation-rect-border: var(--trae-status-error-default);
  --trae-annotation-arrow-anchor-bg: var(--trae-status-primary-default);
...
.trae-browser-inspect-overlay {
  ...
  width: 100%;
  height: 100%;
  z-index: var(--trae-layer-browser-inspect-overlay);
  pointer-events: auto;
...
.trae-browser-inspect-highlight-box {
  ...
  top: 0;
  left: 0;
  z-index: var(--trae-layer-browser-inspect-highlight);
  display: none;
}
...
.trae-browser-inspect-highlight-tag {
  ...
  display: none;
  align-items: center;
  z-index: var(--trae-layer-browser-inspect-action);
}
...
.trae-edit-panel [data-trae-tooltip]::after {
  ...
  left: 50%;
  bottom: calc(100% + 8px);
  z-index: var(--trae-layer-edit-tooltip);
  padding: 6px 10px;
  border-radius: 6px;
...
<nav class="nav">
  <div class="brand">
    ...
    <span>拼读乐园 <span class="en">PhonicsFun</span></span>
  </div>
  <div class="nav-stats">
    <span class="star-count">⭐ <span id="starTotal">11</span></span>
    <div class="progress-mini"><span id="progressBar" style="width: 14.2857%;"></span></div>
    <span id="progressText" style="color:var(--ink-soft)">1/7</span>
  </div>
</nav>
...
<header class="hero">
  ...
  <h1>
    <span class="cn">拼读乐园</span>
    <span class="en">Phonics Fun Park</span>
  </h1>
  <p class="tag">跟着猫头鹰老师，从听音到识字，循序渐进掌握英语自然拼读，做到「见词能读，听音能写」</p>
  <div class="tags">
    <span>🔊 点击即播放</span>
    <span>📈 7级循序渐进</span>
    <span>🎮 趣味互动练习</span>
  ...
  <button class="btn-start" id="btnStart" onclick="startLearning()">
    🚀 开始学习之旅
  </button>
</header>

<!-- 学习路径 -->
<section class="path-section" id="levels">
  <div class="section-title">
    <h2>📚 学习路径</h2>
    <p>从零基础到自主阅读，7个阶段循序渐进，每完成一级解锁下一级</p>
  </div>
  <div class="path-grid" id="pathGrid">
      <div class="level-card lc-1 done " onclick="openLevel(1)">
        <span class="badge">⭐</span>
        <div class="num">1</div>
        <div class="icon">👂</div>
        <h3>音素意识</h3>
        <div class="en-name">Phonemic Awareness</div>
        <p class="desc">先学会「听」——听出首音、押韵和音的拆分合成，这是拼读的地基。</p>
      </div>
      <div class="level-card lc-2  " onclick="openLevel(2)">
        <span class="badge">▶️</span>
        <div class="num">2</div>
        <div class="icon">🔤</div>
        <h3>字母音</h3>
        <div class="en-name">Letter Sounds</div>
        <p class="desc">26个字母最常用的发音。点击每个字母卡片听发音，跟着大声读！</p>
      </div>
      <div class="level-card lc-3  locked" onclick="">
        <span class="badge">🔒</span>
        <div class="num">3</div>
        <div class="icon">📦</div>
        <h3>CVC短词</h3>
        <div class="en-name">CVC Words</div>
        <p class="desc">辅音-元音-辅音结构的短词，用词族归类法学习。换首字母就能读出一串新词！</p>
      </div>
      <div class="level-card lc-4  locked" onclick="">
        <span class="badge">🔒</span>
        <div class="num">4</div>
        <div class="icon">🧩</div>
        <h3>辅音组合</h3>
        <div class="en-name">Consonant Blends</div>
        <p class="desc">两个辅音合在一起发新音。对比 sip/ship，感受组合带来的变化！</p>
      </div>
      <div class="level-card lc-5  locked" onclick="">
        <span class="badge">🔒</span>
        <div class="num">5</div>
        <div class="icon">✨</div>
        <h3>长元音·魔术E</h3>
        <div class="en-name">Magic E</div>
        <p class="desc">词尾不发音的 e 有魔力！它让前面的元音「喊出自己的名字」发长音。</p>
      </div>
      <div class="level-card lc-6  locked" onclick="">
        <span class="badge">🔒</span>
        <div class="num">6</div>
        <div class="icon">🌈</div>
        <h3>词块与高频词</h3>
        <div class="en-name">Word Families &amp; Sight Words</div>
        <p class="desc">元音字母组合发音规律 + 常见不规则高频词。掌握后阅读速度大幅提升！</p>
      </div>
      <div class="level-card lc-7  locked" onclick="">
        <span class="badge">🔒</span>
        <div class="num">7</div>
        <div class="icon">📖</div>
        <h3>句子阅读</h3>
        <div class="en-name">Reading Sentences</div>
        <p class="desc">把学过的词读成句子！点击句子听完整朗读，再自己大声读一遍。</p>
      </div></div>
</section>

<!-- 学习面板 -->
<section class="learn-panel" id="learnPanel">
...
<footer class="footer">
  <div class="ref">
    <strong>📚 教学内容参考来源 / References：</strong><br>
    1. 安丽慧《浅谈自然拼读法与音标教学在小学英语教学中的分阶段运用》(2025)<br>
    2. 邓庆梅《基于"自然拼读"的小学英语语音教学实践与探索——四阶段教学模式》(2025)<br>
    3. 半鱼《自然拼读学习顺序：7个阶段、过关标准与在家练习》(2026)<br>
    4. National Center on Improving Literacy — Phonics: What Is It and Why Is It Important?<br>
    5. 徐俪娜《自然拼读法在小学英语教学中的运用》(北师大外语教学)<br>
    6. 牛津自然拼读世界 Oxford Phonics World 教材体系 (L1-L5)<br>
    7. What Works Clearinghouse — Foundational Skills to Support Reading K-3
  </div>
  <p>拼读乐园 PhonicsFun · 使用浏览器内置语音合成引擎 (Web Speech API) · 适合小学生自然拼读启蒙</p>
</footer>

<script>
// ===== 语音合成（防御性：即使语音API不可用也不影响交互）=====
let synth = null;
let voices = [];
try{
...
function speak(text, rate=0.85){
  try{
    ...
    u.rate = rate;
    u.pitch = 1.1;
    const preferred = voices.find(v=>v.name && v.name.includes('Google US')) || voices.find(v=>v.lang==='en-US') || voices[0];
    if(preferred) u.voice = preferred;
    const ind = document.getElementById('playIndicator');
    if(ind) ind.classList.add('show');
    u.onend = ()=> { if(ind) setTimeout(()=>ind.classList.remove('show'),200); };
    u.onerror = ()=> { if(ind) ind.classList.remove('show'); };
    synth.speak(u);
  }catch(e){
    const ind = document.getElementById('playIndicator');
...
const curriculum = [
  {
    id:1, icon:'👂', cn:'音素意识', en:'Phonemic Awareness',
    color:'#4ea8e8', tip:'先学会「听」——听出首音、押韵和音的拆分合成，这是拼读的地基。',
    type:'awareness',
    items:[
      {q:'听一听，哪个单词以 /b/ 开头？',sound:'bat',options:['bat','cat'],answer:'bat',hint:'bat 蝙蝠'},
      {q:'听一听，哪个单词以 /s/ 开头？',sound:'sun',options:['sun','run'],answer:'sun',hint:'sun 太阳'},
      {q:'bat 和哪个词押韵？',sound:'bat',options:['hat','dog'],answer:'hat',hint:'都结尾 -at'},
      {q:'听一听，哪个单词以 /m/ 开头？',sound:'moon',options:['moon','noon'],answer:'moon',hint:'moon 月亮'},
      {q:'pig 和哪个词押韵？',sound:'pig',options:['dig','cat'],answer:'dig',hint:'都结尾 -ig'},
      {q:'听一听，哪个单词以 /t/ 开头？',sound:'top',options:['top','hop'],answer:'top',hint:'top 顶部'},
    ]
  },
  {
    id:2, icon:'🔤', cn:'字母音', en:'Letter Sounds',
    color:'#ffb800', tip:'26个字母最常用的发音。点击每个字母卡片听发音，跟着大声读！',
    type:'letters',
    items:[
      {letter:'Aa',ipa:'/æ/',word:'apple',emoji:'🍎'},
      {letter:'Bb',ipa:'/b/',word:'ball',emoji:'⚽'},
      {letter:'Cc',ipa:'/k/',word:'cat',emoji:'🐱'},
      {letter:'Dd',ipa:'/d/',word:'dog',emoji:'🐶'},
      {letter:'Ee',ipa:'/e/',word:'egg',emoji:'🥚'},
      {letter:'Ff',ipa:'/f/',word:'fish',emoji:'🐟'},
      {letter:'Gg',ipa:'/g/',word:'goat',emoji:'🐐'},
      {letter:'Hh',ipa:'/h/',word:'hat',emoji:'🎩'},
      {letter:'Ii',ipa:'/ɪ/',word:'igloo',emoji:'🏔️'},
      {letter:'Jj',ipa:'/dʒ/',word:'jam',emoji:'🫙'},
      {letter:'Kk',ipa:'/k/',word:'kite',emoji:'🪁'},
      {letter:'Ll',ipa:'/l/',word:'lion',emoji:'🦁'},
      {letter:'Mm',ipa:'/m/',word:'moon',emoji:'🌙'},
      {letter:'Nn',ipa:'/n/',word:'nose',emoji:'👃'},
      {letter:'Oo',ipa:'/ɒ/',word:'octopus',emoji:'🐙'},
      {letter:'Pp',ipa:'/p/',word:'pig',emoji:'🐷'},
      {letter:'Qq',ipa:'/kw/',word:'queen',emoji:'👑'},
      {letter:'Rr',ipa:'/r/',word:'rabbit',emoji:'🐰'},
      {letter:'Ss',ipa:'/s/',word:'sun',emoji:'☀️'},
      {letter:'Tt',ipa:'/t/',word:'tiger',emoji:'🐯'},
      {letter:'Uu',ipa:'/ʌ/',word:'umbrella',emoji:'☂️'},
      {letter:'Vv',ipa:'/v/',word:'van',emoji:'🚐'},
      {letter:'Ww',ipa:'/w/',word:'watch',emoji:'⌚'},
      {letter:'Xx',ipa:'/ks/',word:'fox',emoji:'🦊'},
      {letter:'Yy',ipa:'/j/',word:'yoyo',emoji:'🪀'},
      {letter:'Zz',ipa:'/z/',word:'zebra',emoji:'🦓'},
    ]
  },
  {
    id:3, icon:'📦', cn:'CVC短词', en:'CVC Words',
    color:'#3cb878', tip:'辅音-元音-辅音结构的短词，用词族归类法学习。换首字母就能读出一串新词！',
    type:'families',
    items:[
      {pattern:'-at',ipa:'/æt/',words:[{w:'cat',e:'🐱'},{w:'bat',e:'🦇'},{w:'hat',e:'🎩'},{w:'mat',e:'🧘'},{w:'rat',e:'🐀'},{w:'sat',e:'🪑'}]},
      {pattern:'-an',ipa:'/æn/',words:[{w:'can',e:'🥫'},{w:'fan',e:'🌀'},{w:'man',e:'👨'},{w:'pan',e:'🍳'},{w:'ran',e:'🏃'},{w:'van',e:'🚐'}]},
      {pattern:'-en',ipa:'/en/',words:[{w:'hen',e:'🐔'},{w:'pen',e:'🖊️'},{w:'ten',e:'🔟'},{w:'men',e:'👥'}]},
      {pattern:'-ig',ipa:'/ɪg/',words:[{w:'big',e:'🐘'},{w:'dig',e:'⛏️'},{w:'fig',e:'🍇'},{w:'pig',e:'🐷'},{w:'wig',e:'💇'}]},
      {pattern:'-og',ipa:'/ɒg/',words:[{w:'dog',e:'🐶'},{w:'fog',e:'🌫️'},{w:'log',e:'🪵'},{w:'bog',e:'🥾'}]},
      {pattern:'-ug',ipa:'/ʌg/',words:[{w:'bug',e:'🐛'},{w:'hug',e:'🤗'},{w:'jug',e:'🫗'},{w:'mug',e:'☕'},{w:'rug',e:'🟫'}]},
    ]
  },
  {
    id:4, icon:'🧩', cn:'辅音组合', en:'Consonant Blends',
    color:'#ff6b4a', tip:'两个辅音合在一起发新音。对比 sip/ship，感受组合带来的变化！',
    type:'families',
    items:[
      {pattern:'sh',ipa:'/ʃ/',words:[{w:'ship',e:'🚢'},{w:'shop',e:'🏪'},{w:'fish',e:'🐟'},{w:'wish',e:'🌟'},{w:'shell',e:'🐚'}]},
      {pattern:'ch',ipa:'/tʃ/',words:[{w:'chip',e:'🍟'},{w:'chop',e:'🔪'},{w:'chat',e:'💬'},{w:'chick',e:'🐤'},{w:'chair',e:'🪑'}]},
      {pattern:'th',ipa:'/θ/',words:[{w:'thin',e:'🥒'},{w:'this',e:'👉'},{w:'bath',e:'🛁'},{w:'math',e:'🔢'},{w:'thumb',e:'👍'}]},
      {pattern:'bl',ipa:'/bl/',words:[{w:'blue',e:'🔵'},{w:'block',e:'🧱'},{w:'blow',e:'💨'},{w:'flag',e:'🚩'}]},
      {pattern:'br',ipa:'/br/',words:[{w:'brown',e:'🟫'},{w:'bread',e:'🍞'},{w:'brush',e:'🖌️'},{w:'brick',e:'🧱'}]},
      {pattern:'st',ipa:'/st/',words:[{w:'star',e:'⭐'},{w:'stop',e:'✋'},{w:'stamp',e:'📮'},{w:'nest',e:'🐦'}]},
    ]
  },
  {
    id:5, icon:'✨', cn:'长元音·魔术E', en:'Magic E',
    color:'#9f7aea', tip:'词尾不发音的 e 有魔力！它让前面的元音「喊出自己的名字」发长音。',
    type:'families',
    items:[
      {pattern:'a_e',ipa:'/eɪ/',words:[{w:'cake',e:'🎂'},{w:'name',e:'📛'},{w:'game',e:'🎮'},{w:'make',e:'🔨'},{w:'cape',e:'🦸'}]},
      {pattern:'i_e',ipa:'/aɪ/',words:[{w:'kite',e:'🪁'},{w:'bike',e:'🚲'},{w:'five',e:'5️⃣'},{w:'nine',e:'9️⃣'},{w:'smile',e:'😊'}]},
      {pattern:'o_e',ipa:'/əʊ/',words:[{w:'home',e:'🏠'},{w:'nose',e:'👃'},{w:'rope',e:'🪢'},{w:'note',e:'📝'},{w:'bone',e:'🦴'}]},
      {pattern:'u_e',ipa:'/juː/',words:[{w:'cube',e:'🧊'},{w:'tube',e:'🧪'},{w:'cute',e:'🥰'},{w:'mule',e:'🐴'}]},
    ],
    compare:[
      {short:'cap',long:'cape',shortC:'帽子',longC:'海角'},
      {short:'pin',long:'pine',shortC:'别针',longC:'松树'},
      {short:'kit',long:'kite',shortC:'工具',longC:'风筝'},
      {short:'hop',long:'hope',shortC:'单脚跳',longC:'希望'},
    ]
  },
  {
    id:6, icon:'🌈', cn:'词块与高频词', en:'Word Families & Sight Words',
    color:'#4ea8e8', tip:'元音字母组合发音规律 + 常见不规则高频词。掌握后阅读速度大幅提升！',
    type:'families',
    items:[
      {pattern:'ai/ay',ipa:'/eɪ/',words:[{w:'rain',e:'🌧️'},{w:'train',e:'🚂'},{w:'play',e:'🎮'},{w:'day',e:'☀️'},{w:'paint',e:'🎨'}]},
      {pattern:'ee/ea',ipa:'/iː/',words:[{w:'tree',e:'🌳'},{w:'green',e:'🟢'},{w:'sea',e:'🌊'},{w:'eat',e:'🍽️'},{w:'leaf',e:'🍃'}]},
      {pattern:'oa',ipa:'/əʊ/',words:[{w:'boat',e:'⛵'},{w:'coat',e:'🧥'},{w:'road',e:'🛣️'},{w:'goat',e:'🐐'}]},
      {pattern:'oo (长)',ipa:'/uː/',words:[{w:'moon',e:'🌙'},{w:'food',e:'🍜'},{w:'spoon',e:'🥄'},{w:'zoo',e:'🦁'}]},
      {pattern:'oo (短)',ipa:'/ʊ/',words:[{w:'book',e:'📖'},{w:'look',e:'👀'},{w:'foot',e:'🦶'},{w:'good',e:'👍'}]},
      {pattern:'ar',ipa:'/ɑː/',words:[{w:'car',e:'🚗'},{w:'star',e:'⭐'},{w:'park',e:'🌳'},{w:'jar',e:'🫙'}]},
    ],
    sight:['the','is','are','was','you','said','have','of','to','do','what','where','come','here','there']
  },
  {
    id:7, icon:'📖', cn:'句子阅读', en:'Reading Sentences',
    color:'#ff6b4a', tip:'把学过的词读成句子！点击句子听完整朗读，再自己大声读一遍。',
    type:'sentences',
    items:[
      {en:'The <hi>cat</hi> sat on a <hi>mat</hi>.',cn:'猫坐在垫子上。',plain:'The cat sat on a mat.'},
      {en:'I see a <hi>blue</hi> <hi>bird</hi>.',cn:'我看见一只蓝色的鸟。',plain:'I see a blue bird.'},
      {en:'She likes to <hi>play</hi> in the <hi>rain</hi>.',cn:'她喜欢在雨中玩耍。',plain:'She likes to play in the rain.'},
      {en:'The <hi>fish</hi> <hi>swims</hi> in the <hi>sea</hi>.',cn:'鱼在海里游泳。',plain:'The fish swims in the sea.'},
      {en:'Look at the <hi>star</hi> in the <hi>sky</hi>!',cn:'看天上的星星！',plain:'Look at the star in the sky!'},
      {en:'He <hi>rides</hi> a <hi>bike</hi> to <hi>home</hi>.',cn:'他骑车回家。',plain:'He rides a bike to home.'},
      {en:'The <hi>moon</hi> is big and <hi>round</hi>.',cn:'月亮又大又圆。',plain:'The moon is big and round.'},
      {en:'We <hi>eat</hi> <hi>bread</hi> and <hi>cake</hi>.',cn:'我们吃面包和蛋糕。',plain:'We eat bread and cake.'},
    ]
  }
];

// ===== 进度管理（防御性：localStorage 在隐私模式可能不可用）=====
let progress = {stars:0, done:[]};
try{
  const saved = localStorage.getItem('phonicsProgress');
  if(saved) progress = JSON.parse(saved);
}catch(e){}
function saveProgress(){
  try{ localStorage.setItem('phonicsProgress', JSON.stringify(progress)); }catch(e){}
}
function updateNav(){
  document.getElementById('starTotal').textContent = progress.stars;
  const doneCount = progress.done.length;
  document.getElementById('progressBar').style.width = (doneCount/7*100)+'%';
  document.getElementById('progressText').textContent = doneCount+'/7';
}

// ===== 渲染学习路径 =====
function renderPath(){
  const grid = document.getElementById('pathGrid');
  grid.innerHTML = curriculum.map((lv,i)=>{
    const done = progress.done.includes(lv.id);
    const locked = i>0 && !progress.done.includes(curriculum[i-1].id);
    const cls = `level-card lc-${lv.id} ${done?'done':''} ${locked?'locked':''}`;
    return `
      <div class="${cls}" onclick="${locked?'':`openLevel(${lv.id})`}">
        <span class="badge">${done?'⭐':(locked?'🔒':'▶️')}</span>
        <div class="num">${lv.id}</div>
        <div class="icon">${lv.icon}</div>
        <h3>${lv.cn}</h3>
        <div class="en-name">${lv.en}</div>
        <p class="desc">${lv.tip}</p>
      </div>`;
  }).join('');
  updateNav();
}
...
function openLevel(id){
  ...
  const themeColor = lv.color;

  header.style.background = `linear-gradient(135deg,${themeColor},${themeColor}dd)`;
  header.innerHTML = `
    <span class="ph-icon">${lv.icon}</span>
    <h2>${lv.cn}</h2>
    <div class="ph-en">${lv.en} · 第 ${lv.id} 关</div>
    <p class="ph-tip">${lv.tip}</p>
    <button class="close-btn" onclick="closePanel()">✕</button>
  `;

  body.innerHTML = renderBody(lv);
  panel.classList.add('show');
  // 等待面板渲染后再滚动，避免被 sticky 导航栏遮挡
  setTimeout(()=>{
    const navHeight = document.querySelector('.nav').offsetHeight;
    const rect = panel.getBoundingClientRect();
    const scrollTop = window.pageYOffset || document.documentElement.scrollTop;
    const target = rect.top + scrollTop - navHeight - 12;
    window.scrollTo({top:Math.max(0,target),behavior:'smooth'});
  },50);
}

function closePanel(){
  document.getElementById('learnPanel').classList.remove('show');
}

// ===== 根据类型渲染内容 =====
function renderBody(lv){
  let html = '';
  if(lv.type==='letters'){
    html += '<div class="sound-grid">';
    lv.items.forEach((it,i)=>{
      html += `
        <div class="sound-card" onclick="playLetter(this,'${it.word}')">
          <div class="letter">${it.letter}</div>
          <div class="ipa">${it.ipa}</div>
          <span class="emoji">${it.emoji}</span>
          <div class="word">${it.word}</div>
          <div class="speak-icon">🔊 点击播放</div>
        </div>`;
    });
    html += '</div>';
    html += renderQuiz(lv);
  }
  else if(lv.type==='families'){
    lv.items.forEach(fam=>{
      html += `
        <div class="word-family">
          <div class="wf-head">
            <span class="wf-pattern">${fam.pattern}</span>
            <span class="wf-ipa">${fam.ipa}</span>
            <button class="wf-play" onclick="playAll('${fam.words.map(w=>w.w).join(' ')}')">🔊 依次播放</button>
          </div>
          <div class="word-list">
            ${fam.words.map(w=>`<span class="word-chip" onclick="speak('${w.w}')"><span class="chip-emoji">${w.e}</span>${w.w}</span>`).join('')}
          </div>
        </div>`;
    });
    if(lv.compare){
      html += `<div class="quiz-title">⚖️ 对比练习：短音 vs 长音</div>`;
      lv.compare.forEach(c=>{
        html += `
          <div class="word-family" style="display:flex;gap:20px;align-items:center;flex-wrap:wrap">
            <span class="word-chip" onclick="speak('${c.short}')" style="background:#ffe0e0">${c.short} (${c.shortC})</span>
            <span style="font-size:24px">→</span>
            <span class="word-chip" onclick="speak('${c.long}')" style="background:#d4f5dd">${c.long}e (${c.longC})</span>
            <span style="font-size:13px;color:var(--ink-soft)">加 e 后元音发长音！</span>
          </div>`;
      });
    }
    if(lv.sight){
      html += `
        <div class="word-family">
          <div class="wf-head"><span class="wf-pattern" style="background:var(--coral)">高频词 Sight Words</span><span class="wf-ipa">需整体记忆</span></div>
          <div class="word-list">
            ${lv.sight.map(w=>`<span class="word-chip" onclick="speak('${w}')" style="background:#fff0e6">${w}</span>`).join('')}
          </div>
        </div>`;
    }
    html += renderQuiz(lv);
  }
  else if(lv.type==='sentences'){
    lv.items.forEach(s=>{
      html += `
        <div class="sentence-card" onclick="speak('${s.plain}',0.75)">
          <div class="sent-en">${s.en}</div>
          <div class="sent-cn">${s.cn}</div>
          <div class="play-hint">🔊 点击听朗读</div>
        </div>`;
    });
    html += renderQuiz(lv);
  }
  else if(lv.type==='awareness'){
    html += `<div class="quiz-title">🎧 听音练习</div>`;
    html += `<p style="color:var(--ink-soft);margin-bottom:16px">点击蓝色按钮听单词发音，再选出正确答案</p>`;
    html += renderQuiz(lv);
  }

  // 完成奖励
  if(!progress.done.includes(lv.id)){
    html += `
      <div class="complete-banner">
        <div class="stars">⭐⭐⭐</div>
        <h3>完成本关挑战！</h3>
        <p>点击下方按钮领取星星，解锁下一关</p>
        <button class="next-btn" onclick="completeLevel(${lv.id})">🎉 领取奖励</button>
      </div>`;
  } else {
    html += `<div class="complete-banner" style="background:linear-gradient(135deg,var(--grass),var(--grass-deep))">
      <div class="stars">✅</div>
      <h3>本关已完成！</h3>
      <p>可以随时回来复习，温故知新</p>
    </div>`;
...
function renderQuiz(lv){
  ...
  if(lv.type==='letters'){
    // 从字母中随机抽4个做选择题
    const shuffled = [...lv.items].sort(()=>Math.random()-0.5).slice(0,5);
    quizzes = shuffled.map(it=>{
      const wrong = lv.items.filter(x=>x.letter!==it.letter).sort(()=>Math.random()-0.5).slice(0,3);
      const opts = [it, ...wrong].sort(()=>Math.random()-0.5);
      return {
        q:`哪个字母开头发 /${it.ipa.replace(/[\/]/g,'')}/ ？听单词 ${it.word}`,
        sound: it.word,
    ...
    // 从词族抽词做选择
    const allWords = [];
    lv.items.forEach(fam=>fam.words.forEach(w=>allWords.push({...w, pattern:fam.pattern})));
    const shuffled = allWords.sort(()=>Math.random()-0.5).slice(0,5);
    quizzes = shuffled.map(w=>{
      const wrong = allWords.filter(x=>x.w!==w.w).sort(()=>Math.random()-0.5).slice(0,3);
      const opts = [w, ...wrong].sort(()=>Math.random()-0.5);
      return {
        q:`听一听，这是哪个单词？`,
        sound: w.w,
        options: opts.map(o=>o.w),
        answer: w.w
      };
    });
  } else if(lv.type==='sentences'){
    const shuffled = [...lv.items].sort(()=>Math.random()-0.5).slice(0,4);
    quizzes = shuffled.map(s=>{
      const wrong = lv.items.filter(x=>x.plain!==s.plain).sort(()=>Math.random()-0.5).slice(0,3);
      const opts = [s, ...wrong].sort(()=>Math.random()-0.5);
      return {
        q:`听句子，选出对应的中文意思`,
        sound: s.plain,
        options: opts.map(o=>o.cn),
        answer: s.cn
      };
    });
  } else if(lv.type==='awareness'){
  ...
  let html = `<div class="quiz-section"><div class="quiz-title">🎯 趣味挑战</div>`;
  quizzes.forEach((q,i)=>{
    html += `
      <div class="quiz-card" id="quiz-${lv.id}-${i}">
        <div class="quiz-q">
          ${i+1}. ${q.q}
          ${q.sound?`<span class="q-sound" onclick="speak('${q.sound}',0.7)">🔊 听</span>`:''}
        </div>
        <div class="quiz-options">
          ${q.options.map(o=>`<div class="quiz-opt" onclick="checkAnswer(${lv.id},${i},'${o}','${q.answer}')">${o}</div>`).join('')}
        </div>
        <div class="quiz-feedback" id="fb-${lv.id}-${i}"></div>
      </div>`;
  });
  html += '</div>';
  return html;
...
// ===== 答题检查 =====
function checkAnswer(lvId,qIdx,selected,answer){
  const card = document.getElementById(`quiz-${lvId}-${qIdx}`);
  const fb = document.getElementById(`fb-${lvId}-${qIdx}`);
  const opts = card.querySelectorAll('.quiz-opt');
  opts.forEach(o=>{
    o.style.pointerEvents='none';
    if(o.textContent===answer) o.classList.add('correct');
    else if(o.textContent===selected && selected!==answer) o.classList.add('wrong');
  });
  if(selected===answer){
    fb.className='quiz-feedback right';
    fb.textContent='✅ 太棒了！答对了！';
    progress.stars += 1;
    saveProgress();updateNav();
  } else {
    fb.className='quiz-feedback wrong';
    fb.textContent=`❌ 再想想，正确答案是「${answer}」`;
  }
}

// ===== 播放字母 =====
function playLetter(card, word){
...
function completeLevel(id){
  ...
  // 庆祝提示
  setTimeout(()=>{
    const next = curriculum.find(l=>l.id===id+1);
    if(next){
      alert(`🎉 恭喜通关第${id}关！\n⭐ 获得5颗星\n下一关「${next.cn}」已解锁！`);
    } else {
      alert(`🎉🎉🎉 恭喜你完成全部7关！\n⭐ 你已掌握自然拼读核心技能！\n继续保持阅读练习吧！`);
...
// 绑定开始按钮（用 addEventListener 替代内联 onclick，移动端更可靠）
const btnStart = document.getElementById('btnStart');
if(btnStart){
  btnStart.addEventListener('click', startLearning);
}
</script>
